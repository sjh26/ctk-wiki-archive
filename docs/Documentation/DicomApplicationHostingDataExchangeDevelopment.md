...
- Ok, not sure about this one: QtSoapArrays are constructed using the *arrayType* attribute from supposedly http://schemas.xmlsoap.org/wsdl/. But it restricts the possible types to a few set, plus *QtSoapType::Other*. This may not be compatible with the *ArrayOf[Type]* way of writing arrays. Extended testing with other implementations should be pursued to get this right.