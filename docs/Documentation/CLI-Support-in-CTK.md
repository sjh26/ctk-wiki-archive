...
Command Line Interface (CLI) support in CTK will primarly focus on providing an API for working with existing CLI modules. Providing utilities and tools for creating CLI modules from scratch is a desired addition but there are no concrete plans for this yet. See [Command_Line_Interface](../Documentation/Command-Line-Interface.html) for more general information about CLI modules.
...
- Run ctkCmdLineModuleFrontend objects
    ...
    1. Customizability
      1. Qt Widget related
See also the [ctkCmdLineModuleFrontendQtGui](http://www.commontk.org/docs/html/classctkCmdLineModuleFrontendQtGui.html) class for customization information.
